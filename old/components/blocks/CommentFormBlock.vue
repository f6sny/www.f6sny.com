	...
	<div class="comments-form">
		...
		<div class="user-info text-left mb-1">
			<div v-if="isAuthenticated">
				<small
					>سيتم نشر التعليق بإسم:
					<mark v-if="isAuthenticated">{{
			...
			<div v-else>
				<small>
					التعليقات مسموحة للزوار، الإسم والبريد الإلكتروني مطلوبين
					للتعليق
				</small>
			</div>
		</div>
	...
	methods: {
		async submit_comment() {
			try {
				...
				const data = await this.$f6snyApi
					.comments()
					.postComment(comment, this.joke.id);
