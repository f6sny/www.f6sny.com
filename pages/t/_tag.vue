<template>
  <div>
      <div v-if="jokes">
        ...
        <br />
        <small class="text-muted">يوجد بالموقع 99999 نكتة تحت تصنيف #{{ tag }}</small></h2>
        <hr />
...
export default {
  ...
  async fetch() {
    const joke_data = await this.$axios.$get(encodeURI(`http://localhost:8080/tags/jokes/${ this.$route.params.tag }?_start=${this.jokes_retreived}`))
